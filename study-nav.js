(()=>{
  ...
  chapter.type='button';
  chapter.setAttribute('aria-label','Open program navigation');
  chapter.setAttribute('aria-expanded','false');
  chapter.innerHTML='<span class="study-chapter-index">01</span><span class="study-chapter-copy"><small>Current section</small><span class="study-chapter-title">The idea</span></span><i class="study-chapter-chevron">⌄</i>';
  ...
  overlay.innerHTML=`
    <div class="study-overlay-head">
      <div><div class="study-overlay-kicker">THE BUILDERS · Al Jadeed Program Concept</div><h2>Choose a section</h2><p>Move directly through the concept, guest universe, episode format, production direction, commercial potential and closing proposal.</p></div>
      <button class="study-overlay-close" type="button" aria-label="Close navigation">×</button>
    </div>
    <nav class="study-overlay-grid" aria-label="All program sections">
      ${items.map(item=>`<a class="study-overlay-link" href="${item.href}" data-target="${item.id}"><span class="num">${item.num}</span><strong>${item.title}</strong><span>Open section ↗</span></a>`).join('')}
    </nav>`;
  ...
  mobileTop.innerHTML=`
    <div class="study-mobile-brand"><span class="study-mobile-mark">ص</span><span class="study-mobile-context"><b>THE BUILDERS</b><span class="study-mobile-section">01 / The idea</span></span></div>
  ...
  const activate=id=>{
    ...
    mobileSection.textContent=item.label;
  ...
  window.addEventListener('scroll',()=>{
    if(tick)return;
    tick=true;
    requestAnimationFrame(()=>{updateProgress();tick=false});
  },{passive:true});
  updateProgress();
